...
### FR-17.4　户型模板快选（v1.5 新增）

没有户型图、也不想手绘的客户，允许从标准模板里选一个"最像我家"的，跳过从零手绘：
...
2. 客户选某个模板后，系统按该模板的标准尺寸（墙长、窗/门/上下水的位置与宽度、
   层高）一次性预填 `FloorPlan`，与手动逐段 `addRun`/`addFeature` 得到的结构
   完全一样——区别只在于这批数据不是客户测的，是模板给的。
3. **预填不算确认**（与 FR-15.5 同一门禁）：每一段墙长、每一处门窗都保留一条
   `needs_confirm`，照样在墙对齐简图上给 Q#、照样出现在追问列表里——客户选模板
   只是给了系统一个更好的起点去问「北墙大概144寸，对吗」，不是替客户报了尺寸。
   `readiness.ts` 的 `windows`/`door`/`walls_ceiling` 关键项**不因为选了模板就
   直接判定 `ok`**。
...
| 6 | 选模板后 `FloorPlan` 的墙长/门窗均带待确认标记，不直接判定检查表 `ok` |
