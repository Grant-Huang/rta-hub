# 加拿大橱柜报价平台 —— 需求说明书（v1.7）

> **v1.7 变更摘要**（户型模板取消数值预填；2026-08-16）：
> 1. **修正 FR-17.4**：户型模板快选不再预填任何数值（墙长、层高、门窗/上下水/
>    燃气/强电的位置宽度）——客户点选"我家像这个"只建墙面**结构**（几段墙、
>    罗盘方位、是否转角相接、是否岛台），这是客户选择动作本身确认过的事实；
>    尺寸类数值系统没有任何依据，一律留空，走跟手动逐段报墙长完全相同的
>    "缺口追问"（而不是"确认门禁"）。
> 2. **背景**：真实会话测试发现，即便预填值标了 `needs_confirm`，也会在
>    `confirmedBriefs`/checklist 等下游环节被误当成"已确认"喂给对话模型，
>    模型进而把模板猜测当事实复述给客户、还跳过该做的确认（详见本次修复的
>    PR）。根源不在"标记有没有打对"，而在"预填这件事本身没有依据"——常见
>    不等于已知，哪怕标成待确认也不该替客户填一个他没提供过的数字。
> 3. **不受影响**：客户真实上传户型图/手绘图走视觉识图（FR-17 主体）不在本次
>    调整范围内——那条路径读的是客户上传的真实图像，读到的值仍然要走
>    `needs_confirm` 确认门禁，只是这份"预填"确有依据。
> 4. `src/samples/README.md` 同步取消每个户型"标注要求"的具体数字，改为
>    列出必填/可后定/可选参数清单，不再暗示这些数字是系统默认值。
>
...
>    新增可点选语义——客户选"我家像这个"即预填标准几何，不必凑空手绘。选"其他"
>    则维持现状（零墙段，走既有的手动补墙追问）。
> 2. 明确**模板预填不是客户确认**：预填的墙长/门窗仍标为待确认（`needs_confirm`），
>    走与 FR-15.5 相同的确认门禁——客户选模板只是给了一个"像不像"的起点，不是
>    量过的尺寸，选完仍要逐项过一遍 Q#。
> 3. 记录**已知限制**：走廊型（galley）两段墙彼此不相接（两面对望，不是转角），
>    现有平面拼接算法（FR-4.3）推不出"两墙相对、中间留过道"这种关系，会退化成
>    「示意排列」——这是拼接算法本身的既有限制，本次不展开修，只如实标注。

> **v1.4 变更摘要**（门窗阻断、门槛分层、确认门禁、拓扑锁定、清单与报价解耦；2026-08-11）：
> 1. 修订 **FR-15.1**：现场检查新增 `door`（此前完全未追踪）；`windows`/`door` 提升为
>    **阻断 `readyToDraw` 的关键项**（此前 `windows` 是非阻断的"缺则继续问"）。理由：
>    柜体避门、水槽居中窗下、吊柜避窗这些排布逻辑（`layout/generate.ts`）早就实现了，
>    检查表却从未要求先问清楚——等于允许系统在不知道门窗位置时就出图。
> 2. 新增 **FR-15.1b 分层门槛**：把单一的 `readyToDraw` 门槛拆成三级——几何/现场/家电
>    仍阻断第一版草图；`province` 改为阻断 `readyToDraw → planReview`（不再阻断第一版
>    草图本身，与 FR-18 通用规格演算保持一致）；`style` 改走 assumed/customer 推定路径
>    （同 `appliances_sizes`），阻断点后移到出完整图纸之后的披露/交付前；`budget` 永不阻断。
> 3. 新增 **FR-15.5 现场关键项确认门禁**：没有安全默认值的关键项（尺寸、门窗），确认
>    方式从"纯聊天文本正则推断"升级为"可见、可纠正的状态"——聊天答案自动勾选，
>    但客户可在 Tab1 手动纠正；门禁查的是状态，不是重新扫聊天原文。
> 4. 修订 **FR-19**：记录已知实现缺口——`resolveWallMention()` 目前只认标签文本/
>    left-right/东南西北/A-D 字母，不认"靠窗那面墙"之类特征指代，也不认数字编号；
>    指代解析不到具体墙段时必须反问，不能默认对齐。
> 5. 新增 **FR-4.3.5 墙段拓扑变更的锁定规则**：厨房形状是从墙段拓扑推出来的，不是
>    客户设的字段（FR-17.2）。转角虚位/岛台/过道校验任一已算出结果并展示给客户之后，
>    改拓扑必须是客户的显式动作；之前可随意调整。
> 6. 修订 **FR-4.2 / 3.7 / FR-6.2**：**产品清单（BOM）与报价单解耦**——通用助理的职责
>    到 `fullDrawings` 为止，出完整图纸时一并产出**不带价格**的产品清单
>    （`layout/bom.ts`）；`quoted` 阶段是给同一份清单叠加价格，不是另生成一份。定价
>    从来只由 `pricing/engine.ts` 纯函数计算，助理不参与——这条边界本次写进对话
>    叙事与阶段职责，而不只是定价代码层面的规则。
...
>    右栏「已确认」是**可读的共识叙事**，不是字段表；系统内部另有检查表，
>    决定还缺什么、能不能问「要不要出设计」。上下水、窗、家电尺寸等**不能猜**。
> 2. 修订 **FR-4.2**：进入 `readyToDraw` 不再「只看户型几何」——必须检查表关键项
>    齐备，并在出图前用**一段文字**把关键事实复述给客户确认。
> 3. 新增 **§3.8 语言策略**：默认英文；仅客户明确要求才切中文；**图纸标注一律英文**。
> 4. 明确主会话输入以自然语言为主，`+` / `@` / `/` 为加速器，不设顶部硬按钮栏。
...
**与产品清单的区别（v1.4 新增）**：`EstimateDraft` 仍然**带金额**，只是用通用目录算的
粗估；`fullDrawings` 阶段产出的产品清单（见 FR-4.2、FR-6.2）**完全不带金额**。两者是
两个独立的轴——一个是「用哪个目录定价」，一个是「要不要定价」，不要混为一谈。

### 3.7 客户界面三栏职责（v1.1 新增）

客户主界面是**一个对话产品**，不是表单外挂聊天。三栏分工：
...
1. **会话负责把世界说清楚；Tab1 负责用可读共识把世界记下来；结构化藏在履约需要处**
   （省份、SKU/门板 id、发送确认）。
...
| `planReview` | 请客户看排布、提意见（进入本阶段前 `province` 必须齐，见 FR-15.1b） | 仅全局俯视图（地柜层 + 吊柜层） |
| `fullDrawings` | 完整图纸已出，附**产品清单**（无价格，`layout/bom.ts`） | 四视图 + 产品清单 |
| `quoted` | 可以发给公司或再比一家 | 四视图 + 产品清单 + **报价单**（同一份清单叠加价格） |

规则：

- 从 `readyToDraw` 到 `planReview` **必须由客户的动作触发**。系统不能因为
  「资料齐了」就自己往前走——那正是这次改造要消除的行为。
- **进入 `readyToDraw` 看内部检查表是否齐备**（FR-15，v1.4 起分三级门槛见 FR-15.1b），
  不只看户型几何。墙长/层高是必要条件，上下水、窗、门、家电尺寸等关键现场事实
  **不能猜**；风格/预算/省份**不在这一步要求**——先用通用规格演算（FR-18），省份在
  客户批准草图前才必须齐，风格在出完整图纸后走推定确认（户型已覆盖尺寸与布局时，
  不再重复要 kitchen size / layout）。
- **产品清单与报价单解耦**（v1.4）：`fullDrawings` 阶段的产品清单只列种类/数量/规格，
  不带任何金额；`quoted` 阶段是给**同一份**清单叠加价格，不是另起一份物料清单，两者
  物料范围必须一致，价格是唯一的增量。助理的对话职责到 `fullDrawings` 为止——报价
  从来只由 `pricing/engine.ts` 纯函数计算（见 3.5 节），助理不参与，也不该在对话里把
  "拿到报价"讲成自己要推进的目标；`quoted` 由履约侧（厂商/ops）触发，见 FR-6。
- **出图前必须用一段自然语言**把检查表上的关键事实复述给客户确认
  （「我们按这些理解开工——对吗？」），得到明确同意后再画。推定值若仍在用，
  复述里必须写明「按推定…」，不得装成已量过。
...
##### FR-4.3.5　墙段拓扑变更的锁定规则（v1.4 新增）

厨房「形状」（一字型/L型/U型/galley）**不是客户设的字段，是从墙段拓扑推出来的**
（见 FR-17.2：形状应从墙段拓扑得出，不问「一字型/L型」）。所以"客户要不要换形状"
在这个系统里等价于"客户要不要改墙段拓扑"——增删一段墙、改变哪两段墙在内墙角相接。

判据不是数墙段数量，是**是否已经有依赖拓扑假设的计算结果被产出并展示给客户**：

- 转角虚位与归属（FR-4.3.2）已经算出；
- 岛台落位/过道净空校验（FR-4.3.4）已经跑过；
- 或任何一版具体排布（哪怕只是 `planReview` 前的草图，不需要已批准）已经生成给客户看过。

在此之前，拓扑还只是假设，改动没有任何东西需要撤销，系统甚至可以按新信息自己
静默重新推导。一旦以上任意一项已经发生，改拓扑就必须是客户的显式动作，且复述里
要说明哪些已算出的结果会被撤销重算（转角柜位置、岛台过道等）——不能像 FR-4.2
警告的"只把轮次计数加一"那样，悄悄换了假设却让客户以为只是画面调整。
...
**与 `fullDrawings` 产品清单的关系（v1.4 新增）**：`src/layout/bom.ts` 产出的这份清单，
`fullDrawings` 阶段就已经给客户看到（不带价格，见 FR-4.2）；`quoted` 阶段的报价单是给
**同一份**清单逐行标价，不是重新生成一份。两者物料范围必须一致，价格是唯一的增量。
...
## 5.x FR-15　设计就绪检查表与设计 brief（v1.1 新增）

> 编号记为 FR-15；正文放在功能需求章末、数据模型章之前。

### FR-15.1　内部检查表（客户不填这张表）

系统为每个会话维护一份**设计就绪检查表**（实现：`src/design/readiness.ts`）。
用途只有两个：

1. 决定对话下一步该澄清什么；
2. 决定能不能进入 `readyToDraw`（「要不要出设计」）。

**不是**给客户填的表单。客户看到的是 Tab1 叙事与会话里的白话。

| 检查项 id | 类别 | 齐备条件 | 不能做什么 |
|---|---|---|---|
| `walls_ceiling` | 几何 | 墙段长度齐、层高齐 | 猜墙长 |
| `plumbing` | 现场 | 至少一段墙有上下水特征，**或**客户明文说「暂无上下水 / 后装」 | 默认塞一个水槽位 |
| `windows` | 现场 | 已标注窗（位置/宽度），**或**客户明文说「这几面墙没有窗」 | 每面墙都发明一扇窗；默认"没有窗" |
| `door`（v1.4 新增） | 现场 | 已标注门洞（位置/宽度），**或**客户明文说「这几面墙没有门」 | 默认某段墙没有门；柜体画到门洞上 |
...
| `style` | 意图 | 需求摘要或偏好里可识别风格，**或**厂商目录默认门板（`provenance: assumed`） | 静默用默认当客户选的 |
| `budget` | 意图 | 需求摘要或 `budgetBand` | — |
| `province` | 意图 | 需求摘要可识别省份（税，且用于厂商分配） | 猜省 |
| `seller` | 履约 | 已选定或 @ 过一家有 published 规格的公司 | — |

每项状态：`ok` | `missing` | `needs_confirm`（有信息但含推定/低置信，出图前必须白话确认）| `deferred`（客户明文推迟；**家电尺寸、窗、门不得 deferred**）。

**关键项分三级门槛**（v1.4 起不再是单一门槛，见 FR-15.1b）：

- **阻断 `readyToDraw`**（第一版草图前）：`walls_ceiling`、`plumbing`、`windows`、`door`、
  `appliances_kinds`、`appliances_sizes`。`windows`/`door` 不能靠系统猜"大概没有"跳过——
  必须是客户明文确认（哪怕答案是"没有"）。`appliances_sizes` 若为推定则状态
  `needs_confirm`，**未确认前不得**进入出图问句。
- **阻断 `readyToDraw → planReview`**（客户点头批准草图前）：加上 `province`。
- **阻断 `fullDrawings` 之后的披露/交付**：加上 `style`（推定路径，未确认只提醒不卡住）。
- **不阻断任何阶段**：`budget`（纯上下文）、`seller`（缺则继续问，不卡流程）。

### FR-15.1b　分层门槛的理由（v1.4 新增）

原先单一门槛把 `style`/`budget`/`province` 和几何/现场事实混在一起要求，导致连第一版
草图都要先问清预算和省份——这与 **FR-18**「无默认厂商时用通用规格先演算」互相矛盾：
既然第一版草图可以不挂靠具体厂商，就不该要求客户在这一步交出只有分配厂商才用得上
的信息。现在按"这项信息在哪一步才真正被用到"分层：

1. **第一版草图**（几何排布）只需要几何、现场特征、家电——直接决定箱体怎么摆，
   缺一项排布就可能是错的（柜子画在门上）。可以用 `GenericCatalog` 通用规格先演算。
2. **客户点头批准草图**（进入 `planReview`）加上 `province`：批准动作意味着"这版排布
   定了"，而厂商按 `serviceAreas` 匹配省份分配；不提前钉死省份，批准的可能是一版
   之后要因换厂商门板/箱体规格而报废的图。
3. **出完整图纸后的披露/交付**加上 `style`：`style` 从一开始就有默认值（厂商 bundle
   第一款门板，或通用目录默认），标 `provenance: assumed`，客户随时可在 Tab1 改；
   只在真要走向交付前提醒是否仍是 assumed，不强制卡住前面步骤。
4. `budget` 永远不阻断——不参与任何计算，只是给助手筛选建议用的上下文。
...
Tab2 列出本会话已产出的：**户型解读图、墙对齐/Q# 图、全局俯视图、四视图、产品清单、
报价单/PDF** 等。产品清单在 `fullDrawings` 阶段即可出现，**不等报价**（v1.4，见 FR-4.2）。
每项可点击预览。进度条可保留，但不得把收集表单塞进输出物栏。

### FR-15.3　出图前文字确认

当检查表关键项均为 `ok`（允许部分 `needs_confirm` 已写入复述）时：

1. 推进到 `readyToDraw`；
2. 助手用**一段话**列出将据以设计的事实（含推定标注）；
3. 问「按这些理解生成设计可以吗？」；
4. 客户明确同意 → `planReview`；说「先等等 / 不对」→ 留在收集并按指出项澄清。

### FR-15.4　与 FR-1.1 / FR-3.2 的关系

- 检查表**不取代**选择题：门板差价、预算档位等仍可用选择题（算得出价格影响时）。
- 家电/水电/窗**优先自然语言**描述；选择题是加速器。答「不确定」合法，但必须进
  `needs_confirm` / 推定标注，不能变成静默正确。

### FR-15.5　现场关键项确认门禁（v1.4 新增）

`windows`/`door`/尺寸等没有安全默认值的关键项（错了直接画错图，不是"体验差"），
确认方式从"纯聊天文本正则推断"升级为"可见、可纠正的状态"：

1. 客户在聊天里给出的答案（含 FR-19 的 Q# 作答）**自动**把对应 Tab1 叙事行标记为
   已确认并变色（复用 3.7 的「已锁定/暂定/未谈及/待澄清」状态色）；
2. 该标记**可由客户手动纠正**——解析错了（比如把答案挂到了错的墙段），客户能在
   Tab1 上直接看到并改，不必"重新说一遍换个措辞"去赌正则能不能命中；
3. 门禁检查的是**标记状态**，不是重新去扫一遍聊天原文；
4. 未标记确认的关键项挡在 `readyToDraw`（见 FR-15.1b）：助手必须在对话里点名列出
   还差哪些 Q#/项（复用 `openItems`/`askHint`），不能只说"资料不全"就不再往下走；
5. `budget`/`style` 等走推定路径、没有"猜错就画错图"风险的字段**不走**本条门禁——
   维持 3.7 的柔性对话收集，不强加勾选交互，避免变成表单。
...
### FR-17.4　户型模板快选（v1.5 新增，v1.7 修正为不预填数值）

没有户型图、也不想手绘的客户，允许从标准模板里选一个"最像我家"的，跳过从零
描述形状，但**不能**跳过报实际尺寸——尺寸没有客户提供的依据，系统不能替他猜：

1. `src/samples/catalog.ts` 里 5 个 `role: "floorplan"` 的示例（单壁型、U型、
   L型+岛台、走廊型、L型标准）**同时承担两个用途**：一是原有的"手绘参考图"
   （点了看示例、照着手绘后上传，见 FR-17 主体），二是本条新增的"选它当起点"。
   两者是同一批图、不同的点击目标，不新增素材文件。
2. 客户选某个模板后，系统只按该模板的**墙面结构**（几段墙、罗盘方位、是否
   转角相接、是否有岛台）建 `FloorPlan` 骨架——这是客户点选这个形状名本身
   就确认过的事实。**不预填任何数值**：每段墙长留 0，不生成门/窗/上下水/
   燃气/强电特征，也不写入层高。这些数值客户从没提供过，哪怕它们是北美
   厨房的常见尺寸，也不代表这位客户家里的真实数据——见 `src/samples/templates.ts`
   顶部注释与 `src/samples/README.md`。
3. **零墙长走"缺口追问"，不是"确认门禁"**：长度为 0 的墙段与手动逐段
   `addRun` 得到的空墙段状态完全一样，走 `site-questions.ts` 的
   `wall_length`（硬缺口）而不是 `wall_confirm`（确认模板猜测）；不生成任何
   `needs_confirm` 待确认项去"确认"一个客户没提供过的数字。`readiness.ts`
   的 `walls_ceiling`/`plumbing`/`windows`/`doors`/`gas`/`electrical` 各项
   照常按"是否已有客户提供/上传识图读到的值"判定 `missing`/`needs_confirm`/`ok`，
   不因为选了模板就多出任何数值。
4. 客户觉得都不像 → 选「其他」，行为等同于**没有**选模板：维持零墙段状态，
   走既有的手动补墙追问（FR-17.2 的例外条款）。不额外新增追问机制，只是不再
   假设某个模板成立。
5. **已知限制**：走廊型（galley）的两段墙彼此不相接（面对面留过道，不是转角
   相接）。现有平面拼接算法（FR-4.3）按"上一段 `endsAtCorner` 且下一段
   `startsAtCorner`"判断相接，两段互不相接时会标注为「示意排列」——这是拼接
   算法本身的既有限制（没有"两墙相对、中间隔多宽过道"这个关系的表达方式），
   本次选模板功能不展开修，如实标注，不假装能画出真实的双排厨房俯视图。
6. **跟视觉识图的边界**：客户真实上传户型图/手绘图时，`template-match.ts`
   仍会用模板的墙面结构去对齐识图结果，且在某个数值字段模型没能读出来时，
   用模板的参考值当"待确认"提示文案里的建议数字——这条路径有客户上传的
   真实图像作为依据，不受本条约束；只有"客户没上传任何图、仅凭点选形状名"
   这条路径（`applyFloorplanTemplate`）禁止预填数值。
...
| 6 | 选模板后 `FloorPlan` 只有墙面结构，墙长为 0、无门窗/上下水特征、层高未设置——不得预填任何数值，也不得因此在检查表 / `confirmedBriefs` 里出现凭空的 `needs_confirm`/`ok` |
| 7 | 选「其他」后系统状态与从未选模板时一致（零墙段、手动补墙追问照常触发） |
...
### FR-19.1　特征指代解析与确认门禁（v1.4 新增）

1. **已知实现缺口**：`resolveWallMention()`（`src/design/chat-site-answers.ts`）目前只认
   墙标签文本、left/right、东南西北方位词、A-D 字母——**不认**"靠窗那面墙""炉子对面
   那段墙"这类基于已知特征的指代，也不认数字编号。指代解析不到具体墙段时，助手必须
   反问澄清，**不能**默认对齐到某一段墙。
2. **Q# 即确认门禁**：客户对某个 Q# 的回答（含"没有"）直接把 FR-15.1 对应检查项
   （`windows`/`door`/尺寸等）的状态由 `missing`/`needs_confirm` 改写为 `ok`，并驱动
   FR-15.5 的 Tab1 变色。
...
| 4 | 客户用特征指代（非标签/方位词/字母）指墙时，解析不到具体墙段则反问，不默认对齐 |
...
> 背景：FR-15.5 已经确立"没有客户依据的数据不能自动算已确认"这条原则
> （FR-17.4 户型模板快选反过来印证了这条原则的边界：没有依据的数值——如
> 客户没提供过的墙长——干脆不预填，连"待确认"都不生成）。这条继续往前走
> 一步：如果输入的数据不是系统猜的，而是体外一套流程（更强的视觉模型读图、
> 专业设计软件的导出、另一套系统的迁移）**已经核实过**的结果，客户不应该
> 被逼着再逐项确认一遍——但"谁说已经核实过"不能只凭一句自称，要有数据
> 说清楚。
...
FR-3.2 的 `ApplianceSpec.provenance`；几何见 FR-15.5 的 `FloorPlanUnresolved`）。
...
2. `provenance: "assumed"`，**或没有声明**：仍要走与 FR-15.5 完全相同的
