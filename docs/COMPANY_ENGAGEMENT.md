...
| 厂商 Agent | `assistant` / `company_agent` | 读交接、开线复述、规格选型、**报 MSRP 参考价（含厂商已发布的标准折扣）** | 低于员工 |
| 厂商员工 | `company_human` | 真人坐席；发言后默认 `agentPaused`；决定标准折扣以外的进一步优惠 | 最高（产品口径） |

**v1.2 起报价规则改了**：厂商 Agent 不再是"绝不报价格"，而是可以引用规格库里已经算好的
MSRP 及标准折扣（`DiscountRule.autoQuotable`，见 `pricing/informal-quote.ts`）——价格必须
在系统里算好，Agent 只转述数字，不允许自己加减乘除或编造清单外的优惠。**总控助手**（不区分
厂商、贯穿主线程的那个 Agent）规则不变，**在主线程里永远不报任何价格数字或区间**，只描述
产品/清单信息，报价的事一律交给 `@厂商名` 之后的厂商 Agent。
