...
- [docs/REQUIREMENTS.md](./docs/REQUIREMENTS.md) —— 需求说明书 v0.8（架构、定价模型、版本化与可追溯、数据模型、FR-1~13、商业模式、开放问题）
...
**MVP-1 ~ MVP-3 均已完成**，598 个测试用例覆盖。端到端可跑：
...
- **M5-5 交付前审核**（`src/delivery/audit.ts`，FR-14）。**在把结果交给客户之前，
  按约束与规则严格过一遍。** 以前检查散在各处，每一处都只看自己那一块，
  **没有任何地方回答「这一份东西现在能不能给客户」**——于是出现过「方案合格
  但物料缺一条踢脚板」「报价算出来了但逐行合计与小计差 3 分」这种组合：
  每一步都成功，结果是错的。阻断项一条都不放行（与硬约束同源，不参与权衡），
  提示项不拦但**必须显示给客户**。报价这一步查得最严（含逐行对账与快照复算），
  不通过返回 409 并说清拦的是什么
- **M5-3/M5-4 家电**：家电是客户的**输入**不是常数；落位在分层前统一决定
  （烟机在吊柜层，位置却由地柜层的灶台定）；留空 = 机身宽 + 通风间隙；
  NKBA 落台区在**落位阶段**就参与决策
...
- **家电配套柜**：冰箱上柜、烟机上柜、烤箱高柜、灶下柜、冰箱侧收口板
  → [docs/APPLIANCES.md](./docs/APPLIANCES.md) §3
